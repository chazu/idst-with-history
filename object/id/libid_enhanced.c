...
/* Old variadic _sendv macro removed - now using numbered macros from libid_enhanced.h */
...
static oop _vtable___alloc_(oop _thunk, oop state, oop self, size_t size)
{
  ...
  dprintf5("_vtable___alloc_(%p, %p, %p, %d) -> %p\n", _thunk, self, state, (int)size, object);
  if (!object) fatal0("out of memory");
  *object++= self;
  dprintf1("  object %p\n", object);
  return (oop)object;
}
...
static oop _object___import_(oop _thunk, oop state, oop self, char *fileName, char *initName)
{
  ...
  dprintf2("dlsym RTLD_DEFAULT %s -> %p\n", initName, init);
  if (!init)
    {
      lib= dlopen(0, ID_RTLD_FLAGS);
      dprintf1("1  dlopen 0 -> %p\n", lib);
      init= dlsym(lib, initName);
      dprintf3("2  dlsym %p %s -> %p\n", lib, initName, init);
      dlclose(lib);
    }
  if (init)
    {
      dprintf1("INTERNAL IMPORT <%s>\n", initName);
    }
  else
    {
...
#      if defined(WIN32)
      ...
      if (!lib)
	{
	  ...
	  lib= dlopen(path, ID_RTLD_FLAGS);
	  if (!lib && !access(path, R_OK|X_OK)) fprintf(stderr, "%s: %s\n", path, dlerror());
	  dprintf2("6  dlopen %s -> %p\n", path, lib);
	}
#      endif
      if (!lib) fatal1("import: %s.so: No such file or directory\n", fileName);
      init= dlsym(lib, "__id__init__");
      dprintf2("7  dlsym %p __id__init__ -> %p\n", lib, init);
      if (!init) fatal1("%s: __id__init__: Undefined symbol\n", path);
    }
  dprintf3("INIT %s %p %p\n", fileName, lib, init);
...
oop _libid_intern(const char *string)
{
  ...
  return _selector___intern_(0, _selector, _selector, string);
}
...
#undef _libid_bind
{
  ...
  dprintf3("_libid_bind(%p<%s>, %p\n", selector, selector->selector.elements, receiver);
  if (!vtable) fatal2("panic: cannot send '%s' to %s: no vtable", selector->selector.elements, nameOf(receiver));
...
# endif
  ...
  assoc= _vtable__lookup_(0, vtable, vtable, selector);
/*assoc= _vtable__lookup_(0, vtable, selector);*/
  ...
  if (!assoc)
	...
	{
	  oop result= _sendv1(s_doesNotUnderstand_, receiver, selector);
	  if (_object___vtable(0, result, result) == _closure->_vtable[-1])
	    return (struct __closure *)result;
	}
      fatal2("primitive error handling failed (%p %p)", receiver, selector);
    }
#if GLOBAL_MCACHE
# if USE_GC
...
struct __lookup _libid_bind2(oop selector, oop receiver)
{
  ...
  do {
    ...
    if (!vtable) fatal2("panic: cannot send '%s' to %s: no vtable", selector->selector.elements, nameOf(fragment));
#  if GLOBAL_MCACHE
    probe= (((unsigned long)vtable << 2) ^ ((unsigned long)selector >> 3)) & (GLOBAL_MCACHE - 1);
    entry= _libid_mcache[probe];
...
#      endif
    ...
    fragment= _sendv0(s__delegate, fragment);
  } while (fragment);
...
oop _libid_proto(oop base)
{
  return _object___delegated(0, (base ? base : _object), (base ? base : _object));
}

oop _libid_proto2(oop base, size_t size)
{
  oop proto;
  dprintf2("_libid_proto2(%p %ld)\n", base, size);
  proto= _object___delegated_(0, (base ? base : _object), (base ? base : _object), size);
  dprintf3("_libid_proto2(%p %ld) => %p\n", base, size, proto);
  return proto;
}
...
oop _libid_import(const char *key)
{
  oop name= _selector___intern_(0, _selector, _selector, key);
  oop assoc= _vtable__findKeyOrNil_(0, _object_Table, _object_Table, name);
  if (!assoc) fatal1("import: '%s' is undefined", key);
...
{
  oop vt= 0;
  dprintf4("_libid_method(%p, %p<%s>, %p)\n", type, selector, selector->selector.elements, method);
  vt= _object___vtable(0, type, type);
  _vtable__methodAt_put_with_(0, vt, vt, selector, (oop)method, 0);
}

oop _libid_alloc(oop type, size_t size)
{
  oop vt= type->_vtable[-1];
  dprintf3("LIBID_ALLOC %p [%p] %ld\n", type, type->_vtable[-1], (long)size);
  return _vtable___alloc_(0, vt, vt, size);
}

oop *_libid_palloc(size_t size)
{
  dprintf1("LIBID_PALLOC %ld\n", (long)size);
#if USE_GC
...
/* Dynamic dispatch functions for runtime argument counting */
oop _libid_sendv_dispatch(oop selector, oop receiver, int argc, oop *argv)
{
    switch(argc) {
        ...
        case 5: return _sendv5(selector, receiver, argv[0], argv[1], argv[2], argv[3], argv[4]);
        default:
...
/* Managed dispatch functions disabled for now - they require _libid to be a pointer */
oop _libid_send_dispatch(oop selector, oop receiver, int argc, oop *argv)
{
    fatal0("_libid_send_dispatch not implemented yet");
    return 0;
}

oop _libid_super_dispatch(oop type, oop selector, oop receiver, int argc, oop *argv)
{
    fatal0("_libid_super_dispatch not implemented yet");
    return 0;
}
...
static void sigint(int signum)	{ fatal0("\nInterrupt"); }
#if defined(SIGHUP)
...
# define method(type, sel, var)								\
  ...
  _vtable__methodAt_put_with_(0, type##_vtable, type##_vtable, s_##var, (_imp_t)type##__##var, 0);
